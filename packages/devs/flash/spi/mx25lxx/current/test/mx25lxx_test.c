...
//      mx25lxx_test.c
//
//      SPI flash driver tests for Macronix MX25Lxx devices and compatibles.
//
//=============================================================================
...
// Date:        2018-07-03
// Purpose:     Macronix MX25Lxx SPI flash driver stress tests.
//
//####DESCRIPTIONEND####
//
//=============================================================================
...
// MX25LXX device is connected to SPI bus 1.  To configure the board for this 
...
// bounce buffers of at least 256 bytes and the MX25LXX read data block size 
...
CYG_DEVS_SPI_CORTEXM_STM32_DEVICE (
    mx25lxx_spi_device, 1, 0, false, 0, 0, 25000000, 1, 1, 1
);
...
CYG_DEVS_FLASH_SPI_MX25LXX_DRIVER (
    mx25lxx_flash_device, 0, &mx25lxx_spi_device
);
  ...
  (void)
{
    ...
    for (i = 0; i < mx25lxx_flash_device.block_info->blocks; i++) {
        base_addr =  mx25lxx_flash_device.start + (i * mx25lxx_flash_device.block_info->block_size);
        ...
        for (j = 0; j < mx25lxx_flash_device.block_info->block_size; j += BUF_SIZE) {
  ...
  (void)
{
    ...
    base_addr =  mx25lxx_flash_device.start;
    for (i = 0; i < (mx25lxx_flash_device.end - mx25lxx_flash_device.start) / 4;) {
        for (j = 0; j < BUF_SIZE;) {
  ...
  (void)
{
    ...
    base_addr = mx25lxx_flash_device.start + mx25lxx_flash_device.block_info->block_size;
    for (i = 1; i <= 256; i++) {
        ...
        base_addr += mx25lxx_flash_device.block_info->block_size;
        if (base_addr >= mx25lxx_flash_device.end)
            base_addr = mx25lxx_flash_device.start + mx25lxx_flash_device.block_info->block_size;
    }
    return errors;
}
...
//-----------------------------------------------------------------------------
// Run all MX25Lxx SPI interface loopback tests.
    ...
    (void)
{
    ...
    if (mx25lxx_flash_device.start == mx25lxx_flash_device.end) {
        diag_printf ("MX25Lxx device not initialised.\n");
        errors ++;
        goto out;
    }
...
out:
    ...
    if (errors == 0)
        CYG_TEST_PASS_FINISH ("MX25Lxx driver tests ran OK.");
    else 
        CYG_TEST_FAIL_FINISH ("MX25Lxx driver test FAILED.");
}
...
{
    do_version();
    diag_printf ("----\nChecking for MX25Lxx compatible devices.\n");
